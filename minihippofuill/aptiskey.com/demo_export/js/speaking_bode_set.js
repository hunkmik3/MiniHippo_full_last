(function () {
    ...
    async function fetchSetData() {
        ...
        // Bài học thử (?demo=1): lấy đề qua API demo công khai, không cần
        // đăng nhập và chỉ đọc được các bộ đề nằm trong allowlist.
        const mhDemo = window.MiniHippoDemo;
        const response = mhDemo && mhDemo.active
            ? await fetch(mhDemo.setUrl(setId))
            : await fetch(`/api/practice_sets/get?id=${encodeURIComponent(setId)}`, {
